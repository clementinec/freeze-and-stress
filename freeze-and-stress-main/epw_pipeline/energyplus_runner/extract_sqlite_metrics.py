...
def extract_tabular_metrics(conn: sqlite3.Connection) -> dict[str, object]:
    ...
    tabular_rows = fetch_all_rows(
        ...
        """
        SELECT
            sr.Value   AS ReportName,
            st.Value   AS TableName,
            srow.Value AS RowName,
            scol.Value AS ColumnName,
            td.Value   AS Value,
            sunits.Value AS Units
        FROM TabularData td
        LEFT JOIN Strings sr
            ON td.ReportNameIndex = sr.StringIndex
        LEFT JOIN Strings st
            ON td.TableNameIndex = st.StringIndex
        LEFT JOIN Strings srow
            ON td.RowNameIndex = srow.StringIndex
        LEFT JOIN Strings scol
            ON td.ColumnNameIndex = scol.StringIndex
        LEFT JOIN Strings sunits
            ON td.UnitsIndex = sunits.StringIndex
        WHERE (sr.Value = 'AnnualBuildingUtilityPerformanceSummary'
            AND st.Value IN ('Building Area', 'Comfort and Setpoint Not Met Summary'))
        OR (sr.Value = 'EnergyMeters'
            AND st.Value = 'Annual and Peak Values - Electricity')
        """,
    )
