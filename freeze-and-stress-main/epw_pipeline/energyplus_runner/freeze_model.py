            ...
            for desc, val in comp_fields.items():
                ...
                if any(
                    ...
                    if "low speed" not in dl:
                        cooling_caps.append(val)
                if any(k in dl for k in ("air flow", "air_flow_rate", "supply air flow")):
                    if "high speed" not in dl and "low speed" not in dl:
                        airflows.append(val)
                elif "maximum flow rate" in dl and "water" not in dl:
                    airflows.append(val)
